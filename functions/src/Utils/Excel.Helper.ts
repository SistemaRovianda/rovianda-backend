   ...
   generatePackingDocumentById(user:User,data:EntrancePacking){
    ...
    async getDailyPreSaleReport(records:DailyReportRecord[]){
        ...
        //    let firstDate = dates[i];
            ...
            worksheet.column(5).setWidth(120);
            worksheet.column(6).setWidth(40);
            worksheet.column(7).setWidth(20);
            worksheet.column(8).setWidth(20);
            worksheet.column(9).setWidth(20);
            worksheet.cell(row,1,row,1,true).string('Fecha').style(style);
            worksheet.cell(row,2,row,2,true).string('Prevendedor').style(style);
            worksheet.cell(row,3,row,3,true).string('Clave cliente').style(style);
            worksheet.cell(row,4,row,4,true).string('Nombre cliente').style(style);
            worksheet.cell(row,5,row,5,true).string('Dirección').style(style);
            worksheet.cell(row,6,row,6,true).string('Contacto').style(style);
            worksheet.cell(row,7,row,7,true).string('Total en KG').style(style);
            worksheet.cell(row,8,row,8,true).string('Total en $').style(style);
            worksheet.cell(row,9,row,9,true).string('Dias de visita').style(style);
            row++;
            let vendedorId="";
            let totalKgGlobal=0;
            ...
            for(let g=indexToStart;g<records.length;g++){
                ...
                worksheet.cell(row,1,row,1,true).string(item.fecha);
                worksheet.cell(row,2,row,2,true).string(item.vendedor);
                worksheet.cell(row,3,row,3,true).string(item.claveCliente?item.claveCliente.toString():"");
                worksheet.cell(row,4,row,4,true).string(item.nombre);
                worksheet.cell(row,5,row,5,true).string(getParseAddress(item));
                worksheet.cell(row,6,row,6,true).string(item.contacto?item.contacto:"");
                worksheet.cell(row,7,row,7,true).string(item.totalKg.toFixed(2));
                worksheet.cell(row,8,row,8,true).string("$"+(+item.totalMonto.toFixed(2)).toLocaleString());
                worksheet.cell(row,9,row,9,true).string(getParseDays(item));
                row++;
                totalKgGlobal+=item.totalKg;
                totalMontoGlobal+=item.totalMonto;
            }
            worksheet.cell(row,7,row,7,true).string('Total: '+totalKgGlobal.toFixed(2));
            worksheet.cell(row,8,row,8,true).string('Total: $'+(+totalMontoGlobal.toFixed(2)).toLocaleString());
            totalKgGlobal=0;
            totalMontoGlobal=0;
        //}
    ...
    async getDailyEffectiveDeliverReport(dateStart:string,dateEnd:string,records:EffectiveDeliverPreSalesReport[]){
            ...
            //let worksheet = workbook.addWorksheet(dateStr);
                ...
                worksheet.cell(3,7,3,7,true).string('Fecha de Entrega').style(style);
                worksheet.cell(3,8,3,8,true).string('Folio Venta').style(style);
                worksheet.cell(3,9,3,9,true).string('Monto Venta').style(style);
                worksheet.cell(3,10,3,10,true).string('Modificó').style(style);
                worksheet.cell(3,11,3,11,true).string('Entregado').style(style);
                let row=4;
